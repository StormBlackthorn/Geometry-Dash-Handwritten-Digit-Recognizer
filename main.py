...
def forward(model_name, X_input=None, iterations=10000):
    if X_input is None:
        X_train, Y_train, X_test, Y_test = clean_data(keras.datasets.mnist.load_data())
    else:
        X_test = X_input.reshape(1, 784)
        Y_test = np.array([0])  # Dummy label
    ...
    for X in X_test:
        ...
        if np.array_equal(prediction, Y_test[total]):
            count += 1
        total += 1
...
def main():
    ...
    forward(model_name, iterations=10000)

    # Example usage with an external image
    #example_image = np.random.randint(2, size=784)  # Replace this with your actual binary image array
    #forward(model_name, X_input=example_image)
